...
static int yt9215_led_serial_remapping_set(struct yt9215_priv *p, int index)
{
	...
	u32 reg = YT9215_LED_SERIAL_REMAPPING_BASE + (2 - slot_led) * 8 + (src_mac / 5) * 4;
	...
	val = (val & ~mask) | ((dst_mac & 0xfU) << (shift + 2)) | ((dst_led & 0x3U) << shift);
...
static int yt9215_led_init(struct yt9215_priv *p)
{
	...
	ret = yt9215_reg_set(p, YT9215_LED_GLB_CTRL, LED_GLB_MODE_MASK, LED_GLB_MODE_SERIAL);
	if (ret)
		return ret;

	ret = yt9215_reg_set(p, YT9215_LED_GLB_CTRL, LED_GLB_SERIAL_PORT_NUM_MASK,
			     LED_GLB_SERIAL_PORT_NUM_7 << LED_GLB_SERIAL_PORT_NUM_OFFSET);
	if (ret)
		return ret;

	/* 2. Active mode = low (bit4); serial pin count = 3 LEDs ([1:0]=2). */
	ret = yt9215_reg_set(p, YT9215_LED_SERIAL_CTRL, LED_SERIAL_ACTIVE_MODE_BIT, LED_SERIAL_ACTIVE_MODE_BIT);
	if (ret)
		return ret;

	ret = yt9215_reg_set(p, YT9215_LED_SERIAL_CTRL, LED_SERIAL_PIN_NUM_MASK, LED_SERIAL_PIN_NUM_3);
	if (ret)
		return ret;
	...
	for (i = 0; i < 10; i++) {
		ret = yt9215_reg_write(p, YT9215_LED_CTRL_0_BASE + i * 4, LED0_ACTION_VALUE);
		if (ret)
			return ret;
	}

	/* 5. Enable the serial LED engine + global LED controller. */
	ret = yt9215_reg_set(p, YT9215_LED_SERIAL_CTRL, LED_SERIAL_ENABLE_MASK, LED_SERIAL_ENABLE_MASK);
	if (ret)
		return ret;

	ret = yt9215_reg_set(p, YT9215_LED_GLB_CTRL, LED_GLB_ENABLE, LED_GLB_ENABLE);
	if (ret)
		return ret;
...
int yt9215_set_default_vlan(struct yt9215_priv *p)
{
	...
	yt9215_reg_write(p, YT9215_VLAN_TBL_REG(1), member << VLAN_TBL_PORT_MEMBER_OFFSET);
	yt9215_reg_write(p, YT9215_VLAN_TBL_REG(1) + 4, untag << VLAN_TBL_UNTAG_OFFSET);

	for (i = 0; i < p->num_ports; i++) {
		mac = yt9215_swport(p, i);
		if (mac < 0)
			continue;
		yt9215_reg_set(p, YT9215_PORT_VLAN_CTRLN_REG(mac), PORT_VLAN_CTRLN_DEFAULT_CVID_MASK,
...
static int yt9215_setup_vlan_mode(struct yt9215_priv *p)
{
	...
	for (i = 0; i < p->num_ports; i++) {
		...
		ret = yt9215_reg_set(p, YT9215_PARSER_PORT_CTRLN_REG(mac),
				     PARSER_PORT_CTRLN_CTAG_TPID_MASK_MASK, PARSER_TPID_PROFILE0);
		if (ret)
			return ret;

		/* ingress admission filter: admit all */
		ret = yt9215_reg_set(p, YT9215_PORT_VLAN_CTRL1N_REG(mac),
				     PORT_VLAN_CTRL1N_CTAG_AFT_MASK | PORT_VLAN_CTRL1N_STAG_AFT_MASK, VLAN_AFT_ALL);
		if (ret)
			return ret;
		...
		ret = yt9215_reg_set(p, YT9215_EGR_PORT_VLAN_CTRLN_REG(mac),
				     ...
				     (u32)EGR_CTAG_MODE_ENTRY_BASED << EGR_PORT_VLAN_CTRLN_CTAG_MODE_OFFSET);
		if (ret)
			return ret;
	}
...
int yt9215_switch_init(struct yt9215_priv *p)
{
	...
	/* Enable MIB counters */
	yt9215_reg_set(p, YT9215_GLOBAL_CTRL1_REG, GLOBAL_CTRL1_MIB_EN, GLOBAL_CTRL1_MIB_EN);

	for (i = 0; i < p->num_ports; i++) {
		if (i == YT9215_CPU_PORT)
			continue;
		yt9215_reg_set(p, YT9215_PORT_CTRL_REG(yt9215_swport(p, i)),
			     PORT_CTRL_FLOW_LINK_AN | PORT_CTRL_AN_LINK_EN | PORT_CTRL_RXMAC_EN | PORT_CTRL_TXMAC_EN,
			     PORT_CTRL_FLOW_LINK_AN | PORT_CTRL_AN_LINK_EN | PORT_CTRL_RXMAC_EN | PORT_CTRL_TXMAC_EN);
	}

	/* 1. Select xMII mode for port 9 (extif_bit=0) */
	yt9215_reg_set(p, YT9215_CHIP_INTERFACE_SEL_REG, CHIP_IFSEL_PORT9_XMII, CHIP_IFSEL_PORT9_XMII);

	/* 2. EXTIF1_MODE: XMII_MODE[31:29]=RGMII(4), XMII_PORT_EN[18]=1 */
	yt9215_reg_set(p, YT9215_EXTIF1_MODE_REG, EXTIF_MODE_XMII_MODE_MASK | EXTIF_MODE_XMII_PORT_EN,
		     ((u32)EXTIF_MODE_RGMII << EXTIF_MODE_XMII_MODE_OFFSET) | EXTIF_MODE_XMII_PORT_EN);
	...
	yt9215_reg_set(p, YT9215_PORT_CTRL_REG(yt9215_swport(p, YT9215_CPU_PORT)),
		     PORT_CTRL_FLOW_LINK_AN | PORT_CTRL_AN_LINK_EN | PORT_CTRL_DUPLEX_MODE |
		     PORT_CTRL_RX_FC_EN | PORT_CTRL_TX_FC_EN | PORT_CTRL_RXMAC_EN | PORT_CTRL_TXMAC_EN |
		     PORT_CTRL_SPEED_MODE_MASK,
		     PORT_CTRL_DUPLEX_MODE | PORT_CTRL_RX_FC_EN | PORT_CTRL_TX_FC_EN |
...
static int yt9215_vlan_carrier_state(struct yt9215_priv *p, u16 vid)
{
	...
	for (i = 0; i < p->num_ports; i++) {
		...
		if (!yt9215_reg_read(p, YT9215_PORT_STATUS_REG(swport), &status) && (status & PORT_STATUS_LINK))
			return 1;
	}
...
/* Link state polling work - OPTIMIZED VERSION
 * 
 * Key improvements:
 * 1. Only sync VLAN carriers when link state actually changes
 * 2. Uses dedicated workqueue to avoid blocking system workqueue
 */
static void yt9215_link_poll_work(struct work_struct *work)
{
	struct yt9215_priv *p = container_of(work, struct yt9215_priv, link_work.work);
	bool changed = false;
	int i;

	/* Sample every user port's MAC link status register */
	for (i = 0; i < p->num_ports; i++) {
		int swport = yt9215_swport(p, i);
		u32 status;
		bool link;
		...
		ret = yt9215_reg_read(p, YT9215_PORT_STATUS_REG(swport), &status);
		if (ret)
			continue;

		link = !!(status & PORT_STATUS_LINK);

		/* Only process if link state changed */
	...
	/* CRITICAL FIX: Only sync VLAN carriers when state actually changed
	 * This prevents RTNL lock contention under high network load
	 */
	if (changed) {
		yt9215_sync_all_vlan_carriers(p);
	}

	/* Re-arm on the dedicated workqueue */
	if (p->link_poll_on) {
		queue_delayed_work(p->link_wq, &p->link_work, msecs_to_jiffies(YT9215_LINK_POLL_MS));
	}
}
...
void yt9215_link_poll_start(struct yt9215_priv *p)
{
	...
	/* Create dedicated workqueue if not exists */
	if (!p->link_wq) {
		p->link_wq = alloc_ordered_workqueue("yt9215_poll", WQ_MEM_RECLAIM);
		if (!p->link_wq) {
			dev_err(p->dev, "Failed to allocate link poll workqueue\n");
			return;
		}
	}

	/* Initialize cached link states */
	for (i = 0; i < p->num_ports; i++) {
		int swport = yt9215_swport(p, i);
		u32 status;
		...
		if (!yt9215_reg_read(p, YT9215_PORT_STATUS_REG(swport), &status))
...
static int yt9215_netdev_event(struct notifier_block *nb, unsigned long event, void *ptr)
{
...
static int yt9215_probe(struct mdio_device *mdiodev)
{
	...
	if (IS_ERR(p->reset_gpio))
		return dev_err_probe(dev, PTR_ERR(p->reset_gpio), "failed to get reset GPIO\n");
	...
	of_property_read_u32(dev->of_node, "motorcomm,cpu-port", &cpu_port);
	of_property_read_u32(dev->of_node, "motorcomm,num-ports", &num_ports);
	p->cpu_port = (u8)cpu_port;
	p->num_ports = (u8)num_ports;

	if (of_property_read_u32_array(dev->of_node, "motorcomm,port-map", map, YT9215_SWCONFIG_PORTS)) {
	...
	if (ret)
		return dev_err_probe(dev, ret, "netdev notifier register failed\n");
...
static void yt9215_remove(struct mdio_device *mdiodev)
{
	...
	/* Destroy dedicated workqueue if exists */
	if (p->link_wq) {
		destroy_workqueue(p->link_wq);
		p->link_wq = NULL;
	}
